	...
	public void Spawn(Transform Parent)
	{
		...
		switch (_OpeningDirection) 
		{
			case OpeningDirection.Top:
				...
				spawnedRoom = Instantiate(_RoomLists.BottomRooms[roomIndex], transform.position, Quaternion.identity, Parent);
				break;
			case OpeningDirection.Bottom:
				roomIndex = Random.Range(0, _RoomLists.TopRooms.Count);
				spawnedRoom = Instantiate(_RoomLists.TopRooms[roomIndex], transform.position, Quaternion.identity,Parent);
				break;
			case OpeningDirection.Right:
				roomIndex = Random.Range(0, _RoomLists.LeftRooms.Count);
				spawnedRoom =  Instantiate(_RoomLists.LeftRooms[roomIndex], transform.position, Quaternion.identity, Parent);
				break;
			case OpeningDirection.Left:
				roomIndex = Random.Range(0, _RoomLists.RightRooms.Count);
				spawnedRoom = Instantiate(_RoomLists.RightRooms[roomIndex], transform.position, Quaternion.identity, Parent);
				break;
		}
		Spawned = true;
